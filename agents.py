...
# ---------------------------------------------------------------------------
#
# ingestion.py already writes a real, stable identifier onto every chunk's
# metadata:
#   - case-law chunks    -> meta["CASE_ID"]           e.g. "Court of Appeal
#                            of Salerno sec. II, 29/12/2022"
#   - civil-code chunks   -> meta["civil_codes_used"]  e.g. "Art. 160"
#                            (a list of strings for case-law docs that cite
#                            multiple articles, a single string for the
#                            civil-code articles themselves)
#
# Label priority:
#   1. CASE_ID metadata for case-law;
#   2. normalized civil_codes_used metadata;
#   3. an article identifier read from the chunk heading;
#   4. the positional fallback "Doc N".
# Metadata is authoritative: a heading is consulted only when citation
# metadata is missing or empty.
# The caller chooses the label from the original chunk first, then removes
# only a duplicated article-ID line while preserving the descriptive title.
#
def _doc_label(meta: dict, fallback_index: int, text: str = "") -> str:
    """Return the best available stable citation label for one chunk."""
    case_id = str(meta.get("CASE_ID") or "").strip()
    if case_id:
        return case_id

    article_label = _normalize_article_metadata(meta.get("civil_codes_used"))
    if article_label:
        return article_label

    heading_label = _label_from_heading(text) if text else None
    if heading_label:
...
# Common metadata variants include "Article 159", "Art. ARTICLE 159",
# "Art 159", and "Art.ART159". Remove at most two repeated prefixes while
# preserving the actual reference (paragraphs, ranges, and "et seq.").
_ARTICLE_NUMBER = r"\d+[A-Za-z]*(?:(?:[-./][A-Za-z0-9]+)|(?:\(\d+\)))*"
_ARTICLE_PREFIX = re.compile(
    r"^(?:Ar\.t\.|Art(?:icle)?s?)\s*\.?\s*"
    r"(?=(?:(?:Ar\.t\.|Art(?:icle)?s?)\s*\.?\s*)?\d)",
    re.IGNORECASE,
)
_BARE_ARTICLE_LABEL = re.compile(rf"^{_ARTICLE_NUMBER}$", re.IGNORECASE)


def _clean_article_label(value: str) -> str:
    """Normalize an article label without rewriting its legal reference."""
    ...
    for _ in range(2):
        match = _ARTICLE_PREFIX.match(remainder)
        if not match:
            break
        candidate = remainder[match.end():].strip()
        if not candidate:
            break
        remainder = candidate
        prefix_removed = True
    ...
    if prefix_removed:
        # Also clean repeated compact tokens such as "ART674, ART677".
        remainder = re.sub(r"\bART(?=\d)", "", remainder, flags=re.IGNORECASE)
        remainder = re.sub(r"\s*,\s*", ", ", remainder)
        return f"Art. {remainder}".strip()

    if _BARE_ARTICLE_LABEL.fullmatch(label):
        return f"Art. {label}"

    # Preserve statute names and other complex references we cannot safely
    # reinterpret, changing only redundant whitespace.
    return label


def _normalize_article_metadata(value: object) -> str:
    """Normalize a string/list civil_codes_used value into one label."""
    if value is None:
        return ""

    values = value if isinstance(value, (list, tuple)) else [value]
    labels = [
        _clean_article_label(str(item))
        for item in values
        if item is not None and str(item).strip()
    ]
    # Deduplicate while retaining the order written by ingestion.
    return ", ".join(dict.fromkeys(label for label in labels if label))


# A heading is a fallback only. It must appear at the start as "ARTICLE N",
# "Art. N", just "N", or an "ACT" line followed by N. Anchoring the
# expression prevents body cross-references from being mistaken for its ID.
_LINE_BREAK = r"(?:\r\n|\n|\r|\u2028|\u2029)"
_ARTICLE_HEADING = re.compile(
    rf"^\s*(?:ACT[ \t]*{_LINE_BREAK}\s*)?"
    rf"(?:(?:ARTICLE|ART\.?)\s+)?(?P<number>{_ARTICLE_NUMBER})\.?"
    rf"[ \t]*(?:{_LINE_BREAK}|$)",
    re.IGNORECASE,
)


def _label_from_heading(text: str) -> Optional[str]:
    """Return a normalized article label from the chunk's first heading."""
    match = _ARTICLE_HEADING.match(text)
    return f"Art. {match.group('number')}" if match else None


# Once the label has been chosen, remove only the repeated identifier line
# from civil-code text. Keep the descriptive title on the following line.
def _strip_redundant_heading(text: str, label: str) -> str:
    """Remove an initial article-ID line when the selected label is an article."""
    if not label.startswith("Art. "):
        return text
    return _ARTICLE_HEADING.sub("", text, count=1)


# Old answers recalled from long-term memory can contain stale "[labels]".
# Parentheses keep the wording readable without making those labels look like
# valid citations retrieved for the current question.
def _defang_bracketed_labels(text: str) -> str:
    """Make bracketed labels in recalled answers non-citable."""
    return text.replace("[", "(").replace("]", ")")


# Fields already represented by the label/context header, or used only for
# internal bookkeeping, are not repeated in the metadata line.
_CONTEXT_LINE_SKIP_KEYS = {
    "CASE_ID", "country", "doc_type", "law", "source",
    "text", "chunk_index", "n_chunks",
}
_CONTEXT_LINE_PLACEHOLDER_VALUES = {"", "no data", "not specified", "n/a", "unknown"}


def _build_metadata_line(meta: dict) -> str:
    """
    Build the metadata text shown with a chunk and included in its grounding
    text. If CASE_ID is the label, cited civil-code articles remain useful
    context and are included in normalized form. Otherwise those articles
    are omitted because they are already represented by the article label.
    """
    case_id = str(meta.get("CASE_ID") or "").strip()
    parts = []
    for k, v in meta.items():
        if k in _CONTEXT_LINE_SKIP_KEYS:
            continue
        if k == "civil_codes_used" and not case_id:
            continue
        if k == "civil_codes_used":
            v = _normalize_article_metadata(v)
        if v in (None, [], {}):
            continue
        if isinstance(v, str) and v.strip().lower() in _CONTEXT_LINE_PLACEHOLDER_VALUES:
            continue
